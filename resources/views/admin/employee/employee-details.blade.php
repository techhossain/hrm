...
<main class="main-content t-mt-15 t-mb-15">
  <div class="container-fluid px-3">
    ...
    <div class="row g-3 mt-1">
      ...
      <div class="col-lg-3">
        <div class="card t-bg-white t-shadow h-100 rounded-0">
          <div class="card-body">

            @if($employee->user->getMedia('dp')->reverse()->first())
            {{ $employee->user->getMedia('dp')->reverse()->first() }}
            @else
            <img src="https://placehold.co/600x400?text={{$employee->user->name}}" alt="{{ $employee->user->name }}"
      ...
      <div class="col-lg-9">
        <div class="card t-bg-white t-shadow h-100 rounded-0">
          <div class="card-body">
            ...
            <p>{{ floor($employee->hourly_rate) }} USD / Hour</p>
            <p><a href="#">{{$employee->slack_username}}</a></p>
            <p class="{{ $employee->status == 1 ? " text-success" : "text-danger" }}">{{$employee->status == 1 ?
              "Active" : "Inactive"}}</p>
          </div>
        </div>
      </div>
          ...
          <div class="col-md-6">
            <div class="card t-bg-white t-shadow h-100 rounded-0">
              <div class="card-body">
                ...
                <div class="row my-1">
                  <div class="col-3 fw-bold">Employee Id</div>
                  <div class="col-9">: EMP-120{{$employee->id}}</div>
                </div>

                <div class="row my-1">
                  <div class="col-3 fw-bold">Full Name</div>
                  <div class="col-9">: {{$employee->user->name}}</div>
                </div>

                <div class="row my-1">
                  <div class="col-3 fw-bold">Designation</div>
                  <div class="col-9">: {{$employee->designation->designation_name}}</div>
                </div>

                <div class="row my-1">
                  <div class="col-3 fw-bold">Department</div>
                  <div class="col-9">: {{$employee->department->department_name}}</div>
                </div>

                <div class="row my-1">
                  <div class="col-3 fw-bold">Skills</div>
                  <div class="col-9">: {{$employee->skills}}</div>
                </div>


                <div class="row my-1">
                  <div class="col-3 fw-bold">Joining Date</div>
                  <div class="col-9">: {{$employee->joining_date}}</div>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="card t-bg-white t-shadow h-100 rounded-0">
              <div class="card-body">
                ...
                <div class="row my-1">
                  ...
                  <div class="col-9">: {{$employee->gender}}</div>
                </div>

                <div class="row my-1">
                  <div class="col-3 fw-bold">Country</div>
                  <div class="col-9">: {{$employee->country->country_name}}</div>
                </div>

                <div class="row my-1">
                  <div class="col-3 fw-bold">Mobile Number</div>
                  <div class="col-9">: {{$employee->mobile}}</div>
                </div>


                <div class="row my-1">
                  <div class="col-3 fw-bold">Birth Date</div>
                  <div class="col-9">: {{$employee->birth_date}}</div>
                </div>

                <div class="row my-1">
                  <div class="col-3 fw-bold">Language</div>
                  <div class="col-9">: {{$employee->language->language_name}}</div>
                </div>

                <div class="row my-1">
                  <div class="col-3 fw-bold">Address</div>
                  <div class="col-9">: {{$employee->address}}</div>
                </div>
              </div>
            </div>
          </div>
